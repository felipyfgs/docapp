...
func (c *NFSeClient) DistDFe(ctx context.Context, ultNSU string) (*NFSeDistResponse, error) {
	...
	if status == 404 {
		return &NFSeDistResponse{CStat: "137", XMotivo: "Nenhum documento localizado"}, nil
	}

	if status >= 400 {
		return nil, fmt.Errorf("ADN API error: status %d, body: %s", status, string(body))
	}

	var resp NFSeDistResponse
	if err := json.Unmarshal(body, &resp); err != nil {
		return nil, fmt.Errorf("parsing ADN response: %w", err)
	}
